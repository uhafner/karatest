...
public class KaraWorld extends World {
    ...
    public static final GreenfootImage ICON_MY_KARA = new Assignment10().getImage();
	...
	private static WorldSetup loadWorldSetupFromFile(String worldFile) {
		...
		try {
			worldSetups = WorldSetup.parseFromFile(worldFile, Assignment10.class, WORLD_SETUP_TITLE_KEY, 
					KARA_DIRECTION_KEY);
		...
		for (int y = 0; y < worldSetup.getHeight(); y++) {
			for (int x = 0; x < worldSetup.getWidth(); x++) {
				...
				case WorldSetup.KARA:
					addObject(new Assignment10(), x, y);
					break;
